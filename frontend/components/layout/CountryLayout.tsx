...
export default function CountryLayout({ name, code, description, benefits, universities, essentials, accentClass = "accent-glow-gold" }: CountryLayoutProps) {
  ...
  return (
    <main className="bg-dark-950 text-white min-h-screen relative overflow-hidden">
      ...
      >
        <Link href="/countries" className="inline-flex items-center gap-4 text-gold-500 font-black uppercase tracking-[0.4em] text-[12px] hover:gap-6 transition-all duration-500 mb-12 group">
          ...
          <motion.div variants={itemVariants} className="space-y-10">
            <div className="flex items-center gap-8">
              ...
              <div className="flex flex-col">
                <span className="text-gold-500 uppercase tracking-[0.5em] font-black text-[11px]">Destination Profile</span>
            ...
            <p className="text-white/60 text-base md:text-lg font-normal leading-relaxed max-w-xl italic border-l-2 border-gold-500/10 pl-8 py-4">
              {description}
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 pt-6">
               {[
                 ...
                 <div key={i} className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-gold-500/5 hover:border-gold-500/10 transition-all group">
                    <span className="text-[11px] text-white/50 group-hover:text-gold-500 uppercase font-black tracking-[0.2em] transition-colors">{fact.label}</span>
                    <p className="text-sm font-bold text-white transition-colors">{fact.value}</p>
                 </div>
               ))}
            </div>
          </motion.div>
                ...
                <ul className="space-y-5">
                  {benefits.map((benefit, i) => (
                    ...
                    >
                      ...
                      <span className="text-white/70 group-hover/item:text-gold-500 text-sm font-medium transition-colors uppercase tracking-[0.1em]">{benefit}</span>
                    </motion.li>
                  ))}
                </ul>
             </motion.div>
             ...
             >
                ...
                <div className="flex flex-wrap gap-3">
                  {universities.map((uni, i) => (
                    <span key={i} className="px-5 py-2 bg-white/5 rounded-full text-[12px] font-black uppercase tracking-[0.1em] text-white/60 hover:bg-gold-500 hover:text-black transition-all cursor-default translate-gpu hover:scale-105">
                      {uni}
                    </span>
                  ))}
                </div>
