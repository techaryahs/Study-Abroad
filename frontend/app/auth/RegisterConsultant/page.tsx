...
const RegisterConsultant = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 selection:bg-gold-500 selection:text-black font-sans relative overflow-hidden">
      ...
      <AnimatePresence>
        {verifyModal.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-[#0f1115] border border-white/10 p-7 rounded-[28px] w-full max-w-[320px] relative shadow-2xl text-center">
      ...
      <div className="bg-[#0f1115]/90 backdrop-blur-3xl border border-white/10 rounded-[32px] shadow-2xl w-full max-w-5xl flex overflow-hidden relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <div className="relative group">
                        ...
                        <input name="name" value={form.name} onChange={handleChange} placeholder="Dr. John Doe" maxLength={50} className="w-full pl-10 pr-3 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 transition-colors" />
                      </div>
                    </div>
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <div className="relative group">
                        ...
                        <input name="role" value={form.role} onChange={handleChange} placeholder="Senior Admissions Counselor" maxLength={50} className="w-full pl-10 pr-3 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 transition-colors" />
                      </div>
                    </div>
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <div className="relative group">
                        ...
                        <input name="email" value={form.email} onChange={handleChange} placeholder="email@address.com" className={`w-full pl-10 pr-20 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none transition-colors ${isEmailVerified ? 'border-green-500/50' : 'focus:border-gold-500'}`} />
                    ...
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <div className="relative group">
                        ...
                        <input type={showPassword ? "text" : "password"} name="password" value={form.password} onChange={handleChange} placeholder="••••••••" minLength={6} maxLength={32} className="w-full pl-10 pr-10 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 transition-colors" />
                ...
                <motion.div key="2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <input name="expertise" value={form.expertise} onChange={handleChange} placeholder="e.g. Ivy League, MBA..." maxLength={100} className="w-full px-4 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 transition-colors" />
                    </div>
                    <div className="col-span-2 md:col-span-1">
                      <label className="block text-[9px] font-black text-gray-500 uppercase tracking-[2px] mb-1.5 ml-1">Experience</label>
                      <select name="experience" value={form.experience} onChange={handleChange as any} className="w-full px-4 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 appearance-none">
                    ...
                    <div className="col-span-2">
                      ...
                      <textarea name="bio" value={form.bio} onChange={handleChange} placeholder="Describe your methodology and success rate..." maxLength={500} className="w-full px-4 py-2.5 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white outline-none focus:border-gold-500 transition-colors min-h-[60px] resize-none" />
                    </div>
                    <div className="col-span-2">
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <div className="flex-1 relative">
                           ...
                           <div className="w-full px-3 py-2 bg-white/[0.03] border border-white/10 rounded-lg flex items-center gap-2 text-gray-400 hover:border-gold-500 transition-colors">
                  ...
                  <div className="p-4 bg-white/[0.03] border border-white/5 rounded-xl">
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {DAYS_OF_WEEK.map(day => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider border transition-all ${selectedDays.includes(day) ? 'bg-gold-500 border-gold-500 text-black shadow-[0_0_10px_rgba(234,179,8,0.2)]' : 'bg-transparent border-white/10 text-gray-500 hover:border-white/30'}`}
                        >
                          {day.slice(0, 3)}
                        </button>
                      ))}
                    ...
                    <div className="flex items-end gap-3 mb-4">
                      <div className="flex-1">
                        ...
                        <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-3 py-2 bg-white/[0.05] border border-white/10 rounded-lg text-xs text-white outline-none focus:border-gold-500 [color-scheme:dark]" />
                      </div>
                      <div className="flex-1">
                        <label className="block text-[9px] font-black text-gray-500 uppercase tracking-[2px] mb-1.5 ml-1">End Time</label>
                        <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-3 py-2 bg-white/[0.05] border border-white/10 rounded-lg text-xs text-white outline-none focus:border-gold-500 [color-scheme:dark]" />
                      </div>
