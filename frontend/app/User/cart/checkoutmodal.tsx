    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-[#362B25]/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-[#FFFFFF] w-full max-w-sm max-h-[90vh] rounded-[2rem] shadow-2xl overflow-hidden border border-[#D4A848]/20 relative animate-in zoom-in-95 duration-300 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden p-6 sm:p-8">
          ...
          {/* Wallet Section */}
          <div className={`bg-[#F8F6F1] border border-[#D4A848]/10 rounded-2xl p-4 group transition-all duration-300 ${showWalletError ? 'border-red-500/30' : 'hover:border-[#D4A848]/30'}`}>
          ...
          <div className="h-6 flex items-start mb-2 px-1">
            {showWalletError && (
              <p className="text-[8px] text-red-500 font-extrabold uppercase tracking-widest flex items-center gap-1.5 animate-in slide-in-from-top-1 duration-200 mt-2">
                <AlertCircle size={10} className="shrink-0" />
                Error: Please add balance first
              </p>
            )}
          </div>
