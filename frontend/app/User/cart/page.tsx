...
export default function CartPage() {
  ...
  const handleAppleIapCheckout = async () => {
    const token = getToken();
    if (!token) {
      router.push("/auth/login");
      return;
    ...
    alert(
      "Apple In-App Purchase flow will be connected here for iOS. Razorpay is disabled on iOS for now.",
    );
    return;
  };

  const finalTotal = cart.reduce((total, item) => total + (item.price || 0), 0);
  const actualSubtotal = cart.reduce(
    (total, item) => total + (item.actualPrice || item.price / 0.8),
    0,
  );
  ...
  const getServiceIcon = (serviceId: string) => {
    if (serviceId.includes("visa"))
      return <Globe size={28} className="text-[#362B25]/60" />;
    if (serviceId.includes("sop") || serviceId.includes("essay"))
      return <FileText size={28} className="text-[#362B25]/60" />;
    ...
    <main className="min-h-screen bg-[#FDFBF7] text-[#362B25] pt-12 md:pt-20 pb-24 relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-8">
          <h1 className="text-4xl md:text-[42px] font-bold font-serif text-[#362B25] tracking-normal uppercase border-b-[3px] border-[#D4A848]/60 inline-block pb-4 px-2">
            My Cart
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            {cart.length === 0 ? (
              <div className="bg-white border border-[#D4A848]/20 rounded-2xl p-12 md:p-24 flex flex-col items-center justify-center text-center shadow-sm">
                <div className="w-48 h-48 bg-[#F8F6F1] rounded-2xl flex items-center justify-center mb-10 border border-[#D4A848]/10 group hover:scale-105 transition-transform">
                  <div className="relative">
                    ...
                    <Search
                      size={32}
                      className="absolute bottom-4 right-4 text-[#D4A848]/40"
                    />
                  </div>
                </div>
                <h2 className="text-2xl font-bold text-[#362B25]/40 mb-8 lowercase first-letter:uppercase">
                  Your cart is empty.{" "}
                  <Link
                    href="/services"
              ...
              <div className="space-y-6">
                {cart.map((item) => (
                  <div
                    key={item.itemId}
                    className="bg-white border border-[#D4A848]/20 rounded-md p-5 md:p-6 shadow-sm relative group transition-all hover:shadow-md"
                  >
                    <div className="flex gap-4 md:gap-6 items-start">
                      ...
                      <div className="flex-1 w-full min-w-0">
                        <div className="flex flex-col sm:flex-row justify-between gap-2 md:gap-4 lg:pr-4">
                          <h3 className="text-lg md:text-[20px] font-bold font-serif text-[#362B25] leading-snug flex-1">
                            {item.title}
                          </h3>
                          <div className="text-left sm:text-right shrink-0 mt-1 sm:mt-0">
                            <p className="text-[14px] font-bold md:text-xs font-bold text-[#675F5B]/50 line-through mb-0.5">
                              {currency}{" "}
                              {formatPrice(
                                item.actualPrice || item.price / 0.8,
                              )}
                            </p>
                            <p className="text-[18px] md:text-[22px] font-black text-red-700 leading-none">
                        ...
                        <p className="text-[12px] md:text-[13px] text-[#675F5B] leading-relaxed max-w-2xl mt-2 md:mt-3 font-medium">
                          {item.description ||
                            "Expert help to boost your profile and secure admissions to top Ivy League and global universities with personalized mentorship."}
                        </p>

                        {/* Options like CoAuthors if applicable */}
                        {(item.serviceId.includes("research") ||
                          item.title.toLowerCase().includes("research")) && (
                          <div className="flex items-center gap-3 mt-3 md:mt-4">
                            <span className="text-[13px] font-bold md:text-[14px] font-bold font-black uppercase tracking-widest text-[#675F5B]">
                              CoAuthors:
                            </span>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-white border border-[#D4A848]/20 rounded-xl overflow-hidden shadow-sm">
              <div className="p-6">
                <h3 className="text-xl font-bold font-serif text-[#362B25] text-center mb-8">
                  Order Summary
                </h3>
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-[15px] text-[#362B25]">
                    <span>Total Amount:</span>
                    <span className="text-[#675F5B]/50 line-through">
                      {currency} {formatPrice(actualSubtotal)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-[15px] text-[#362B25]">
                    <span>Total Discount:</span>
                    <span className="text-red-700 font-bold">
                      - {currency} {formatPrice(totalDiscount)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border border-[#D4A848]/20 rounded-2xl p-10 shadow-sm">
              <h3 className="text-lg font-black text-[#362B25] mb-10 uppercase tracking-[0.1em] opacity-40">
                You may also like
              </h3>
              <div className="space-y-12">
                ...
                >
                  ...
                  <div>
                    <h4 className="text-xs font-black text-[#362B25] uppercase tracking-tight mb-2 leading-tight group-hover:text-blue-600 transition-colors">
                      Visa Application Help
                    </h4>
                    <p className="text-[14px] font-bold text-[#675F5B] leading-relaxed font-semibold opacity-70">
                      Ace the visa application through our help in paperwork,
                      financial planning, and visa interview mock rounds.
                    </p>
                  </div>
                </Link>
                <Link
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-bold text-[#675F5B] leading-relaxed font-semibold opacity-70">
                      The EB-1 visa is a talent-based immigrant visa in the US
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-bold text-[#675F5B] leading-relaxed font-semibold opacity-70">
                      Identify the universities that match your profile and
                      career goals with our expert vetting process.
                    </p>
                  </div>
                </Link>
                <Link
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-bold text-[#675F5B] leading-relaxed font-semibold opacity-70">
                      Draft a compelling, admissions-driven narrative that
                      highlights your unique journey to the committee.
                    </p>
                  </div>
                </Link>
                <Link
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-bold text-[#675F5B] leading-relaxed font-semibold opacity-70">
                      Publishing credible research papers can significantly
                      boost your profile for MS/PhD applications.
                    </p>
                  </div>
                </Link>
              </div>
