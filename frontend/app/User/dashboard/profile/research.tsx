...
export default function Research({ onClose }: ResearchProps) {
  ...
  return (
    <div className="w-[750px] mx-auto bg-white rounded-xl overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[500px] relative font-sans animate-in fade-in zoom-in duration-300 border border-gray-100">
      {/* LEFT PANEL */}
      <div className="w-[33%] bg-[#FFB300] p-8 flex flex-col items-center justify-center text-center gap-6 relative">
        <div className="w-[100px] h-[100px] bg-[#C5E1A5] rounded-full flex items-center justify-center shadow-inner relative overflow-hidden">
          {/* Custom Research Icon from Screenshot */}
          <div className="relative w-full h-full flex items-center justify-center">
             {/* Colorful Pie Sections */}
              ...
              <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6 pt-2">
                ...
                <div className="space-y-2 relative" ref={searchRef}>
                  ...
                  {!isAddingContributors ? (
                    <button onClick={() => setIsAddingContributors(true)} className="text-[#2196F3] font-bold text-[14px] flex items-center gap-1 hover:underline transition-all">+ Add contributors</button>
                  ) : (
                        ...
                        {showContributors && (
                          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute z-50 w-full mt-1 bg-white border border-[#E0E0E0] rounded-md shadow-xl max-h-[300px] overflow-y-auto">
                            {filteredContributors.map(c => (
                              <div key={c.id} onClick={() => { toggleContributor(c.id); setShowContributors(false); setContributorQuery(""); setIsAddingContributors(false); }} className="px-4 py-3 hover:bg-gray-50 cursor-pointer flex items-center gap-4 border-b border-gray-50 last:border-0 transition-colors">
              ...
              <motion.div key="step3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center h-full space-y-6 py-6 text-center">
                <div className="w-24 h-24 text-[#212121]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <path d="M12 10l-1.5 1.5L9 10" className="opacity-0" /> {/* Just to balance */}
        ...
        {showSuccess && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-[100] flex items-center justify-center p-8">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[10px] p-10 max-w-[400px] w-full text-center shadow-2xl">
              <div className="text-[#212121] text-[15px] font-bold mb-8 italic">Your details has been added successfully</div>
              <button onClick={onClose} className="w-[120px] bg-[#FFFF00] hover:bg-[#FFFF33] text-black font-bold py-3 rounded-full transition-all border-[1.5px] border-[#212121] mx-auto block active:scale-95 shadow-sm text-[13px]">Close</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
