              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setFormData({ ...formData, isOngoing: !formData.isOngoing })}>
                    <div className={`w-5 h-5 rounded border-2 transition-all ${formData.isOngoing ? 'bg-[#4CAF50] border-[#4CAF50]' : 'border-white/10 group-hover:border-white/30'}`}>
                      {formData.isOngoing && <CheckCircle size={16} className="text-[#0a0a0a]" />}
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Ongoing Mission</span>
                  </div>
                </motion.div>
              )}
              {step === 3 && (
