...
export default function TestScores({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="w-[750px] mx-auto bg-white rounded-xl overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[500px] relative font-sans animate-in fade-in zoom-in duration-300">
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-50 flex items-center justify-center p-8"
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-white rounded-[10px] p-10 max-w-[400px] w-full text-center shadow-2xl"
            >
              <div className="text-[#212121] text-[15px] font-medium mb-8">
                Your details has been added successfully
              </div>
              <button
                onClick={onClose}
                className="w-[120px] bg-[#FFFF00] hover:bg-[#FFFF33] text-black font-bold py-3 rounded-full transition-all border-[1.5px] border-[#212121] mx-auto block active:scale-95 shadow-sm text-[13px]"
              >
                Close
              </button>
            </motion.div>
