...
const ROADMAP_STEPS = [
  { id: 1, title: "Kickstart Your Journey", description: "Submit your basic academic and financial profile to begin the assessment.", icon: <Compass className="w-6 h-6" /> },
  { id: 2, title: "Connect with a Counselor", description: "Expert consultation to understand your specific funding needs and eligibility.", icon: <PhoneCall className="w-6 h-6" /> },
  { id: 3, title: "Get Personalized Lender Choices", description: "Get matched with trusted lenders, curated by our professionals for your profile.", icon: <ShieldCheck className="w-6 h-6" />, active: true },
  { id: 4, title: "Mark the Milestone", description: "Smooth documentation support and final disbursement to your university.", icon: <Flag className="w-6 h-6" /> }
];

const LENDER_COMPARISON = [
  { category: "PSB", icon: <Building2 className="w-5 h-5" />, maxAmount: "Up to ₹2 Crore", interestRate: "9.25% - 11.30%", collateral: "No (Up to 50 Lakh)", tenure: "15 years", processingFee: "₹10,000" },
  { category: "Private Banks", icon: <Building2 className="w-5 h-5 text-blue-600" />, maxAmount: "Up to ₹2 Crore", interestRate: "9.50% - 13.50%", collateral: "No (Up to 1 Crore)", tenure: "15 years", processingFee: "0.50% - 1%" },
  { category: "NBFCs", icon: <Coins className="w-5 h-5 text-orange-500" />, maxAmount: "Up to ₹1.5 Crore", interestRate: "10% - 14%", collateral: "No (Up to 75 Lakh)", tenure: "15 years", processingFee: "1% - 2%" },
  { category: "International Lenders", icon: <Globe2 className="w-5 h-5 text-green-600" />, maxAmount: "Up to $200,000", interestRate: "10.5% - 14%", collateral: "No", tenure: "15 years", processingFee: "1% - 5%" }
];

const PARTNER_HIGHLIGHTS = [
  { bank: "SBI", logo: "/sbi-logo.png", type: "PSB", maxLoan: "Up to ₹3 Cr", interestRate: "9.15% - 10.15%", processingTime: "15 - 20 Days", processingFee: "₹10,000", loanType: "Collateral & Non - Collateral" },
  { bank: "Union Bank", logo: "/union-bank-logo.png", type: "PSB", maxLoan: "Up to ₹1.5 Cr", interestRate: "9.20% - 10.50%", processingTime: "12 - 18 Days", processingFee: "Up to ₹20,000", loanType: "Collateral" }
];

const BENEFITS = [
  { title: "We Advocate for You", desc: "We work for you, not the banks, helping you choose from lenders that truly fit your background and goals." },
  { title: "Smart Interest Rates", desc: "Access exclusive student offers and negotiated rates that you won't find on public lender pages." },
  { title: "Transparent Process", desc: "From start to finish, we make it easy, with no consultation fees, no jargon, just straightforward guidance." }
];
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
              <p className="text-[#000000] text-lg font-medium max-w-lg">
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-xs py-2 border-b border-[#F1EDEA]">
                    {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
                    <span className="text-[#000000] font-medium">Max. Loan Amount</span>
                    <span className="text-[#2D2926] font-bold">Up to 7.5 lakhs</span>
                  </div>
                  <div className="flex justify-between items-center text-xs py-2 border-b border-[#F1EDEA]">
                    {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
                    <span className="text-[#000000] font-medium">Interest Rate (%)</span>
                    <span className="text-[#2D2926] font-bold">11.50% - 12.25%</span>
                  </div>
                  <div className="flex justify-between items-center py-2">
                    {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
                    <span className="text-[#000000] text-xs font-medium">Monthly EMI</span>
            ...
            <table className="w-full text-left border-separate border-spacing-x-4 border-spacing-y-0">
                  ...
                  {LENDER_COMPARISON.map((col) => (
                    <th key={col.category} className="py-6 px-8 text-center bg-[#FDFBF7] rounded-t-3xl border-x border-t border-[#C5A059]/10 min-w-[200px]">
                      <div className="flex flex-col items-center gap-2">
                        <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center">{col.icon}</div>
              ...
              <tbody className="bg-white">
                {[
                  ...
                  <tr key={row.label}>
                    {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
                    <td className="py-6 px-4 text-sm font-bold text-[#000000] border-b border-[#F1EDEA]">{row.label}</td>
                    {LENDER_COMPARISON.map((col) => (
      ...
      <section id="partners" className="px-6 md:px-14 lg:px-20 py-24 bg-[#1C1917] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* CHANGED: text-white/50 -> text-white (on dark bg, full white is readable) */}
          ...
          <div className="lg:col-span-3 flex gap-6 overflow-x-auto no-scrollbar pb-10">
            {PARTNER_HIGHLIGHTS.map((p, i) => (
              <div key={i} className="flex-shrink-0 w-80 bg-white rounded-[32px] p-8 space-y-8 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 px-6 py-2 bg-[#C5A059]/10 text-[#C5A059] text-[10px] font-black tracking-widest uppercase rounded-bl-3xl">{p.type}</div>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-[#FDFBF7] flex items-center justify-center border border-[#C5A059]/10 font-black text-[#C5A059]">{p.bank[0]}</div>
              ...
              {BENEFITS.map((b, i) => (
                <div key={i} className="flex gap-4 group">
                  ...
                  <div className="space-y-1">
                    ...
                    {/* CHANGED: text-[#6B5E51] -> text-[#000000] */}
                    <p className="text-[#000000] font-medium leading-relaxed">{b.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="px-6 md:px-14 lg:px-20 py-24">
        <div className="max-w-7xl mx-auto bg-[#2D2926] rounded-[48px] p-12 md:p-24 text-center space-y-10 shadow-[0_40px_100px_rgba(0,0,0,0.2)] relative overflow-hidden border border-white/5">
          ...
          <div className="space-y-6 relative z-10">
            ...
            {/* CHANGED: text-white/50 -> text-white on dark background */}
            <p className="dm text-white text-xl font-medium max-w-2xl mx-auto leading-relaxed">
