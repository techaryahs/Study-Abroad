...
const verifyApplePayment = async (req, res) => {
  try {
    ...
    // Placeholder only.
    // Real Apple receipt / transaction verification will be implemented later.
    // For now, do NOT mark Apple IAP as paid and do NOT create a paid receipt.

    return res.status(501).json({
      success: false,
      message:
        "Apple IAP verification is not implemented yet. Connect real App Store purchase validation before enabling iOS paid checkout.",
...
const getMyReceipts = async (req, res) => {
  try {
    ...
    const receipts = await Receipt.find({ userEmail: email }).sort({
      createdAt: -1,
    });
    res.json(receipts);
  } catch (err) {
